		...
		Couchdb.prototype.sync.call( self, factory, function(err){
			...
			self.elasticsearch.sync( factory, function(err){
				...
				self.client.deleteRiver( self.index, factory.resource, function(){
					...
					if ( self.config.auth && self.config.auth.username ) river.couchdb.user = self.config.auth.username;
					if ( self.config.auth && self.config.auth.password ) river.couchdb.password = self.config.auth.password;
