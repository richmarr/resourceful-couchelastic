...
resourceful.Resource.join = function( resources, callback ){
	
	var entity_field = this.resource.toLowerCase(),
		id_field = entity_field+'_id',
...
function listKeys( resources, id_field ){
	...
	while( count-- ) joins.push( resources[count][id_field] );
	return joins;
}
